...
#pos = position
#tile = the tile number that calls that specific tile type
...
class Map_Engine:
    ...
    def load_map(file):
        ...
        for tile in range(len(map_data)):
            ...
            tiles.append(map_data[tile].split(":")) # Split position from the texture identifier number

        for tile in tiles:
            tile[0] = tile[0].split(",") #split position into list
            pos = tile[0]
            for p in pos:
